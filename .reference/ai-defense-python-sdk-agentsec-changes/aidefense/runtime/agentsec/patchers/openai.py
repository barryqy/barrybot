...
def _resolve_response_for_inspection(response: Any) -> Any:
    """Parse raw OpenAI responses so post-call inspection can read completion text."""
    try:
...
def _normalize_kwargs_for_strict_openai_compat(instance: Any, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    """For APIs that reject extra fields (e.g. Mistral): use max_tokens, drop max_completion_tokens and other unsupported params."""
    client = getattr(instance, "_client", None)
    if not client:
        return kwargs
...
def _wrap_chat_completions_create(wrapped, instance, args, kwargs):
    ...
    # Call the original (normalize params for strict APIs like Mistral)
